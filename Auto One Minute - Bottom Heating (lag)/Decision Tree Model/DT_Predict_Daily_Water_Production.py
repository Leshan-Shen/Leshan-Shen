...
#long running
#%% Read DT_Predict_Water_Production.csv
DT_Predict_Water_Production = pd.read_csv('DT_Predict_Water_Production.csv',index_col = 0) #Read csv file
#%%
DT_Predict_Water_Production['TIMESTAMP'] = pd.to_datetime(DT_Predict_Water_Production['TIMESTAMP'])
ID0_Clean_Data = DT_Predict_Water_Production[DT_Predict_Water_Production['ID'] == 0]
ID2_Clean_Data = DT_Predict_Water_Production[DT_Predict_Water_Production['ID'] == 2]
    ...
    for value in ID0_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                DT_Predict_Weight_Values = ID0_Clean_Data['Predict_Net_Weight'][ID0_Clean_Data['TIMESTAMP'] == value].values[0]
                ID0March_Data.append(DT_Predict_Weight_Values)
...
for i in range(1,31):
    ...
    for value in ID0_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                DT_Predict_Weight_Values = ID0_Clean_Data['Predict_Net_Weight'][ID0_Clean_Data['TIMESTAMP'] == value].values[0]
                ID0April_Data.append(DT_Predict_Weight_Values)
...
for i in range(1,32):
    ...
    for value in ID2_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                DT_Predict_Weight_Values = ID2_Clean_Data['Predict_Net_Weight'][ID2_Clean_Data['TIMESTAMP'] == value].values[0]
                ID2March_Data.append(DT_Predict_Weight_Values)
...
for i in range(1,31):
    ...
    for value in ID2_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                DT_Predict_Weight_Values = ID2_Clean_Data['Predict_Net_Weight'][ID2_Clean_Data['TIMESTAMP'] == value].values[0]
                ID2April_Data.append(DT_Predict_Weight_Values)
...
#%%
ID0_DT_Predict_Daily_Water_Production = np.row_stack((ID0Mar_Daily_Water_Production,
                                               ID0Apr_Daily_Water_Production
                                               ))
ID2_DT_Predict_Daily_Water_Production = np.row_stack((ID2Mar_Daily_Water_Production,
                                               ID2Apr_Daily_Water_Production
                                               ))
ID0_DT_Predict_Daily_Water_Production = pd.DataFrame(ID0_DT_Predict_Daily_Water_Production,
                                           columns = ['TIMESTAMP','Water_Production'])
ID0_DT_Predict_Daily_Water_Production.insert(0,'ID',0)
ID2_DT_Predict_Daily_Water_Production = pd.DataFrame(ID2_DT_Predict_Daily_Water_Production,
                                           columns = ['TIMESTAMP','Water_Production'])
ID2_DT_Predict_Daily_Water_Production.insert(0,'ID',2)

DT_Predict_Daily_Water_Production = pd.concat([ID0_DT_Predict_Daily_Water_Production,ID2_DT_Predict_Daily_Water_Production],sort=True).reset_index(drop=True)

#%%
DT_Predict_Daily_Water_Production.to_csv('DT_Predict_Daily_Water_Production.csv')
