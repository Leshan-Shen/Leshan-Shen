...
#long running
#%% Read Net_Water_Production.csv
Net_Water_Production = pd.read_csv('Net_Water_Production-2021-03-29-2021-04-25.csv',index_col = 0) #Read csv file
#%%
Net_Water_Production['TIMESTAMP'] = pd.to_datetime(Net_Water_Production['TIMESTAMP'])
ID0_Clean_Data = Net_Water_Production[Net_Water_Production['ID'] == 0]
ID2_Clean_Data = Net_Water_Production[Net_Water_Production['ID'] == 2]
    ...
    for value in ID0_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                Net_Weight_Values = ID0_Clean_Data['Net_Weight'][ID0_Clean_Data['TIMESTAMP'] == value].values[0]
                ID0March_Data.append(Net_Weight_Values)
...
for i in range(1,31):
    ...
    for value in ID0_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                Net_Weight_Values = ID0_Clean_Data['Net_Weight'][ID0_Clean_Data['TIMESTAMP'] == value].values[0]
                ID0April_Data.append(Net_Weight_Values)
...
for i in range(1,32):
    ...
    for value in ID2_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                Net_Weight_Values = ID2_Clean_Data['Net_Weight'][ID2_Clean_Data['TIMESTAMP'] == value].values[0]
                ID2March_Data.append(Net_Weight_Values)
...
for i in range(1,31):
    ...
    for value in ID2_Clean_Data['TIMESTAMP']:
        if value >= record_date:
            if value <= record_date_plus_one:
                Net_Weight_Values = ID2_Clean_Data['Net_Weight'][ID2_Clean_Data['TIMESTAMP'] == value].values[0]
                ID2April_Data.append(Net_Weight_Values)
...
#%%
ID0Auto_Daily_Water_Production = np.row_stack((ID0Mar_Daily_Water_Production,
                                               ID0Apr_Daily_Water_Production
                                               ))
ID2Auto_Daily_Water_Production = np.row_stack((ID2Mar_Daily_Water_Production,
                                               ID2Apr_Daily_Water_Production
                                               ))
ID0Auto_Daily_Water_Production = pd.DataFrame(ID0Auto_Daily_Water_Production,
                                           columns = ['TIMESTAMP','Water_Production'])
ID0Auto_Daily_Water_Production.insert(0,'ID',0)
ID2Auto_Daily_Water_Production = pd.DataFrame(ID2Auto_Daily_Water_Production,
                                           columns = ['TIMESTAMP','Water_Production'])
ID2Auto_Daily_Water_Production.insert(0,'ID',2)

Auto_Daily_Water_Production = pd.concat([ID0Auto_Daily_Water_Production,ID2Auto_Daily_Water_Production],sort=True).reset_index(drop=True)

#%%
Auto_Daily_Water_Production.to_csv('Auto_Daily_Water_Production.csv')
